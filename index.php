...
define('PATH', 'tmp'); //relative folder where the images are saved
...
{
    $p = PATH . '/';
    if ((MAX_EXEC === 0 || (time() - INIT_EXEC) < MAX_EXEC) && //prevents this function locks the process that was completed
        (file_exists($p) === true || is_dir($p) === true)) {
        $h = opendir($p);
        if (false !== $h) {
            while (false !== ($f = readdir($h))) {
                if (is_file($p . $f) === true && is_dir($p . $f) === false && strpos($f, SECPREFIX) !== false && (INIT_EXEC - filectime($p . $f)) > (CCACHE * 2)) {
                    unlink($p . $f);
                }
            }
        }
...
function downloadSource($url, $toSource, $caller)
{
    ...
    if ($caller > MAX_LOOP) {
        return array(
            'error' => 'the limit of ' . MAX_LOOP . ' redirects was exceeded, maybe there is a problem: ' . $url
        );
    }
    $uri    = parse_url($url);
    ...
    if ($fp === false) {
        return array(
            'error' => 'SOCKET: ' . $errstr . '(' . ((string) $errno) . ')'
        );
    } else {
        ...
        while (false === feof($fp)) {
            if (MAX_EXEC !== 0 && (time() - INIT_EXEC) >= MAX_EXEC) {
                return array(
                    'error' => 'Maximum execution time of ' . ((string) (MAX_EXEC + 5)) . ' seconds exceeded, configure this with ini_set/set_time_limit or "php.ini" (if safe_mode is enabled)'
                );
            }
            $data = fgets($fp);
            if ($data === false) {
            ...
            if ($isBody === false) {
                if (preg_match('#^location[:]#i', $data) !== 0) { //200 force 302
                    ...
                    if ($data === '') {
                        return array(
                            'error' => '"Location:" header is blank'
                        );
                    }
                    $nextUri = $data;
                    ...
                    if ($data === '') {
                        return array(
                            'error' => 'Invalid scheme in url (' . $nextUri . ')'
                        );
                    }
...
if (isset($_SERVER['HTTP_HOST']) === false || strlen($_SERVER['HTTP_HOST']) === 0) {
    ...
    if ($tmp === false) {
        $err      = get_error();
        $response = array(
            'error' => 'Can not create file' . ($err !== null && isset($err['message']) && strlen($err['message']) > 0 ? (': ' . $err['message']) : '')
        );
        $err      = null;
    } else {
...
if (is_array($response) && isset($response['mime']) && strlen($response['mime']) > 0) {
    ...
    if (false === file_exists($tmp['location'])) {
        ...
        $response['mime'] = str_replace(array(
            'windows-bmp',
            'ms-bmp'
        ), 'bmp', //mimetype bitmap to bmp extension
        ...
        if (rename($tmp['location'], $locationFile)) {
            ...
            $response = array(
                'error' => 'Failed to rename the temporary file'
            );
        }
    }
}
